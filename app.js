...
function renderM(m){
  ...
  const _queueRowFn=function(e,i){
    ...
    return '<tr style="cursor:pointer'+(e.contentChanged?';background:rgba(220,38,38,.03);border-left:3px solid #dc2626':'')+'" onclick="if(!event.target.closest(\'input,button,.bi,.st\')){openApproval('+e.id+')}">'+
      ...
      '<td style="font-size:10px;max-width:200px;white-space:normal;word-break:break-word">'+(e.note||'')+(e.locked?'':'<span style="cursor:pointer;margin-left:4px;opacity:.4;font-size:9px" title="Sửa nội dung 編輯內容" onclick="event.stopPropagation();quickEditNote('+e.id+')">✏️</span>')+(e.contentChanged?'<div style="margin-top:3px;font-size:9px;color:#dc2626;font-weight:600">⚠️ Đã sửa nội dung — cần duyệt lại</div>':'')+prepaidTag+qAdvTag+'</td>'+
      ...
      '<td>'+(e.contentChanged?'<span class="st" style="background:rgba(220,38,38,.08);color:#dc2626;border:1px solid rgba(220,38,38,.2)" onclick="event.stopPropagation();openApproval('+e.id+')">Cần duyệt lại</span>':'<span class="st '+st.c+'" onclick="event.stopPropagation();openApproval('+e.id+')">'+st.l+'</span>')+'</td>'+
  ...
  const _readOnlyRowFn=function(e,i){
    ...
    const qAdvObj=hasAdvPayerInList(e.payer)&&e.advStaff?STAFF.find(s=>s.code===e.advStaff):null;
    const qAdvTag=hasAdvPayerInList(e.payer)?'<div style="margin-top:3px;background:'+(e.advPaid?'rgba(16,185,129,.15)':'rgba(251,191,36,.15)')+';color:'+(e.advPaid?'#34d399':'#fbbf24')+';padding:3px 8px;border-radius:6px;font-size:9px;font-weight:600;border:1px solid '+(e.advPaid?'rgba(110,231,183,.3)':'rgba(252,211,77,.3)')+'">💰 TƯ: '+(qAdvObj?qAdvObj.name:e.advStaff)+' | '+fmtV(e.advAmt||0)+(e.advPaid?' ✓':' ✗')+'</div>':'';
    return '<tr style="cursor:pointer'+(e.contentChanged?';background:rgba(220,38,38,.03);border-left:3px solid #dc2626':'')+'" onclick="if(!event.target.closest(\'input,button,.bi\')){openApproval('+e.id+')}">'+
      '<td><input type="checkbox" '+(e.selected&&e._selCtx===_currentQCtx?'checked':'')+' onchange="event.stopPropagation();toggleOne('+e.id+',\''+_currentQCtx+'\')"/></td>'+
      '<td style="text-align:center;color:var(--g400);font-size:10px">'+(i+1)+'</td>'+
      '<td style="font-size:10px;font-weight:500">'+(e.code||'')+'</td>'+
      '<td style="font-size:10px">'+ds+wfDate+'</td>'+
      ...
      '<td>'+(e.contentChanged?'<span class="st" style="background:rgba(220,38,38,.08);color:#dc2626;border:1px solid rgba(220,38,38,.2)">Cần duyệt lại</span>':'<span class="st '+st.c+'">'+st.l+'</span>')+'</td>'+
  ...
  if(!filt.length){if(tbody)tbody.innerHTML='';if(emptyEl)emptyEl.style.display='';}
    ...
    if(tbody){tbody.innerHTML=filt.map((e,i)=>{
      ...
      const _chgStyle=e.contentChanged?'cursor:pointer;background:rgba(220,38,38,.03);border-left:3px solid #dc2626':'cursor:pointer';
      return `<tr style="${_chgStyle}" onclick="if(!event.target.closest('input,button,select,.bi,.st')){openApproval(${e.id})}">
